...
window.onload = function () {
     $.getScript('http://int.dpool.sina.com.cn/iplookup/iplookup.php?format=js', function(_result) {
            if (remote_ip_info.ret == '1') {
               ...
               city_now = remote_ip_info.city;
               jsonp(createUrl(city_now)[0]);
               jsonp(createUrl(city_now)[1]);
            } else {
...
function getTodayWeather(response) {
    ...
    else if (data[0].pm25 <  100) {
        var air = '良';
    }
    else if (data[0].pm25 <  150) {
        var air = '轻度污染';
    }
    else if (data[0].pm25 <=  200) {
        var air = '中度污染';
    }
    else if (data[0].pm25 >  200) {
        var air = '重度污染';
...
function createUrl() {
    ...
    var urls = [];
    urls[0] = 'https://sapi.k780.com/?app=weather.future&appkey=32530&sign=4c528e3bd4851768dc5d68e12730d7d7&format=json&jsoncallback=getWeather&weaid=' + encodeURI(cityName);
    urls[1] = 'https://api.map.baidu.com/telematics/v3/weather?output=json&ak=qoDuRtmVFSvBrAFhZAaIFPkOXBaFLo2d&callback=getTodayWeather&location=' + encodeURI(cityName);
    return urls;
}